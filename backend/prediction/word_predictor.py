...
_LEGACY_BACKEND_DIR = os.path.dirname(_MODULE_DIR)
_WORD_LIST_CANDIDATES = [
    os.path.join(_MODULE_DIR, "wordlist.txt"),
    os.path.join(_LEGACY_BACKEND_DIR, "wordlist.txt"),
]
...
class WordPredictor:
    ...
    def __init__(self, word_list_path=None):
        self.words = set()
        path = word_list_path or self._resolve_word_list_path()
        self._load_words(path)

    def _resolve_word_list_path(self):
        for candidate in _WORD_LIST_CANDIDATES:
            if os.path.exists(candidate):
                return candidate
        return _WORD_LIST_CANDIDATES[0]
